...
public class Producer implements Runnable{
    ...
    public Producer(BlockingDeque<String> queue){
        ...
        try {
//            fundCode = Files.readAllLines(Paths.get(Producer.class.getResource("/fundCode.txt").getPath())).stream()
//                    .map(id->Long.valueOf(id))
//                    .collect(Collectors.toList());
            fundCode = FundCodeList.getFundCodeList();
            ...
            try {
                timeList.forEach(time->{
                    ...
                    if(timeId<(sleepTime/60/1000))
                        fundCode.forEach(id -> writeQueue.add(id));
                });
